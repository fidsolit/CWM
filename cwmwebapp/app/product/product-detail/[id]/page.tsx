...
export default function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, isLoading } = useSWR(
    resolvedParams.id ? `/gettingProductbyID/${resolvedParams.id}` : null,
    () => get_product_by_id(resolvedParams.id)
  );
  ...
  const AddToCart = async () => {
    if (!resolvedParams.id || !user?._id) {
      toast.error("You must login first .");
      return;
    }

    const finalData = { productID: resolvedParams.id, userID: user._id };
  ...
  const AddToBookmark = async () => {
    if (!resolvedParams.id || !user?._id) {
      toast.error("Unable to bookmark. Please login first.");
      return;
    }
        ...
        <ul>
          ...
          <li>
            ...
            >
              ...
              {prodData?.productCategory?.categoryName ||
