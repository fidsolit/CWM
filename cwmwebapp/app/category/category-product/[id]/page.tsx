...
export default function Page({
  ...
  params: Promise<pageParam>; // Params is now a Promise
  searchParams: any;
}) {
  const [thisProduct, setThisProdData] = useState<ProductData[] | []>([]);
  ...
  useEffect(() => {
    async function fetchParams() {
      ...
      setCategoryId(resolvedParams.id); // Set the `id` value
    }
    fetchParams();
  }, [params]);

  const { data, isLoading } = useSWR(
    categoryId ? `/gettingProductOFSpecificCategoryID/${categoryId}` : null,
  ...
  return (
    ...
    <div className="text-sm breadcrumbs md:mt-20 "> 
        <ul>
          <li>
            <Link href={"/"}>
              <svg
