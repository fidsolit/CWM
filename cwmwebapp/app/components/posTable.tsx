...
export default function POSDataTable() {
  const { mutate } = useSWRConfig();
  const router = useRouter();
  const [prodData, setprodData] = useState<POS[] | []>([]);
  const data = useSelector((state: RootState) => state.Admin.product);
  const isLoading = useSelector(
    (state: RootState) => state.Admin.productLoading
  );
  const [search, setSearch] = useState("");
  const [filteredData, setFilteredData] = useState<POS[] | []>([]);
  ...
  const columns = [
    {
      name: "Product Name",
      selector: (row: POS) => row?.productName,
      sortable: true,
    },
    {
      name: "Category",
      selector: (row: POS) => row?.productCategory?.categoryName,
      sortable: true,
    },
    {
      name: "Available Quantity",
      selector: (row: POS) => row?.productQuantity,
      sortable: true,
    },
    {
      name: "Image",
    ...
    },
    {
      ...
      cell: (row: POS) => (
        <div className="flex items-center justify-start px-2 h-20">
          <button
            onClick={() => router.push(`/product/update-product/${row?._id}`)}
